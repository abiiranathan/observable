...
template <typename T>
class OBSERVABLE_EXPORT Observable {
   public:
    using Observer = std::function<void(const T&)>;
    Observable<T>() {}
    void subscribe(const Observer& observer) { observers.push_back(observer); }
...
template <typename T>
class OBSERVABLE_EXPORT DataStore {
   public:
    DataStore<T>() {}
    ...
    Observable<T>& getObservable() { return observable; }
...
class OBSERVABLE_EXPORT QSettingsWrapper : public QObject {
   ...
   public:
    ...
    // Returns a reference to the underlying settings object
